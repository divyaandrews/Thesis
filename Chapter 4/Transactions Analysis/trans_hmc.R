...
Date <- Date[1:460]
model <- stan_model('~/Documents/Paper3/R files/trans_noge.stan')
options(mc.cores=4)
fit <-suppressWarnings( sampling(model,list(N=N,Y=Y),algorithm="NUTS", iter = 1000,warmup=500, chains=4,thin=1))
s<- summary(fit)
...
ebic_nogp <- -2*(sum(col_means))/4 + 4*log(368)

# Assuming your Stan fit is done
...
lambda_368_samp <- posterior$cmean[,368]
...
# Initial values
x_prev <- Y[368]
lambda_prev <- lambda_368_samp  # Vector of length G
...
# Create the plot
plot_1<- ggplot(plot_long, aes(x = Time, y = Value, color = Type, shape = Type)) +
