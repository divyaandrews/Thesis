...
#Import data
data_1 <- scan(file.choose()) #EricssonB_Jul2.txt
Tlen <- length(data_1)
Tlen #460

start <- as.POSIXct("2002-07-02 09:35:00")
end <- start + 24*60*60
Date <- seq(start, end, by = "1 mins")

Date <- Date[1:460]

data_train <- data.frame(Date[1:368], data_1[1:368])
colnames(data_train) <- c("Date", "data_1")
data_1<-data_train$data_1


maxval <- max(data_train$data_1)
maxval #37


breaks_start <- as.POSIXct(format(start, "%Y-%m-%d"))
breaks_end <- as.POSIXct(format(end, "%Y-%m-%d"))
#Line plot of time series
plot1<- ggplot(data=data_train, aes(x= Date, y=data_1, group=1)) +
  geom_line(color = "darkred")+
  geom_point(size=0.2)+
  labs(y= "Transaction counts ", x = "Time") +
  scale_x_datetime(
    breaks = seq(breaks_start, breaks_end, by = "40 mins"),
    date_labels = "%H:%M"
  )

acf_values <- acf(data_train$data_1, plot = FALSE,lag.max=60)
pacf_values <- pacf(data_train$data_1, plot = FALSE)
# Create a data frame including lag 0
acf_df <- data.frame(
  Lag = acf_values$lag[1:60],
  ACF = acf_values$acf[1:60]
)
# Create a data frame including lag 0
pacf_df <- data.frame(
  Lag = pacf_values$lag[1:20],
