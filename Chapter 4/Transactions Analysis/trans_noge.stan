...
transformed parameters {
  ...
  real beta1 = 0.84 * inv_logit(beta1_unconstd); // Transformed beta1
...
model {
  // Priors for alpha0
  alpha0 ~ lognormal(-1.2446, 0.3);
  // Priors for theta
  phi_unconstd ~ normal(-0.1205, 30);
  
  // Bivariate normal prior on transformed unconstrained parameters
   vector[2] mu = [ -1.6458, 3.1889]';           // Mean vector
...
generated quantities {
  ...
  cmean_last = cmean[N];  // Save the last conditional mean
}
